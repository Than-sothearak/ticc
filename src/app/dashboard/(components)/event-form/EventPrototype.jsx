...
export const EventPrototypeForm = ({ data }) => {
  ...
  const handleRemoveImage = (index) => {
    setFormData((prev) => {
      const removed = prev.prototypes[index];
      return {
        ...prev,
        prototypes: prev.prototypes.filter((_, i) => i !== index),
  ...
  const handleSubmit = async (e) => {
    ...
    startTransition(async () => {
      ...
      try {
        const res = await fetch("/api/past-event/event-prototype", {
          method: method,
          body: formDataToSend,
        });
        ...
        setFormData((prev) => ({
          ...prev,
          prototype: data.event.prototypes || [],
          removedImages: [],
        }));
        alert(data.message);
        setIsUpload(false);
      ...
      <Card className="w-full">
        <CardHeader>
          <div className="flex justify-between">
            <CardTitle>Add prototype images</CardTitle>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid lg:grid-cols-5 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-3">
            {formData.prototypes.map((img, index) => (
              <div
                key={index}
